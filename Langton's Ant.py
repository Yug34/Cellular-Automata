...
grid_size = (640, 320)  # Sets the size of the grid
cell_size = 2  # Sets the drawn size of each cell
frame_rate = 60
calc_per_frame = 750  # Sets the calculations done between each frame - higher is faster
...
def update(generation, start_time):
    pygame.display.set_caption('Langton\'s Ant: Generation {}  Time: {}  Generations Per Second: {}'.format(str(generation), round(time.time() - start_time, 2), round(generation / round(time.time() - start_time, 2), 2)))
